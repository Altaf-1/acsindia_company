...
class StudyMaterialPaymentController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->merchant_id_study = config('services.hdfc.merchant_id_study');
        $this->working_key_study = config('services.hdfc.working_key_study');
    }
    ...
    {
        $course = StudyMaterial::where('slug', $course)->get()->first();
        $user = Auth::user();
        ...
        if(StudyBank::where('study_material_id', $course->id)
        ...
        $data['merchant_id'] = $this->merchant_id_study;
        $data['redirect_url'] = route('hdfc.payment.study-material.response');
        $data['cancel_url'] = route('hdfc.payment.study-material.response');
        $data['language'] = 'en';
        ...
        $working_key = $this->working_key_study;
    ...
    public function complete_pending_order($order_id): Factory|\Illuminate\View\View|Application
    {
        ...
        $course = StudyMaterial::where('id', $order->course_id)->get()->first();
        $user = Auth::user();
    ...
    public function response(Request $request): RedirectResponse
    {
        try{
            ...
            $workingKey = $this->working_key_study;
            ...
            $course = StudyMaterial::findOrFail($hdfcOrder->first()->course_id);
            ...
            $user->study_materials()->attach($hdfcOrder->first()->course_id);
            ...
            Invoice::create([
              ...
              'mode' => 'HDFC',
                'course' => StudyMaterial::findOrFail($hdfcOrder->first()->course_id)->title,
