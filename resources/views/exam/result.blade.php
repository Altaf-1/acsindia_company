...
<!-- end header area -->
        ...
        <div class="container">
            <div class="row">
                <div class="lernen_banner_title">
                    <h2 class="text-white">{{$test_name}}</h2>
                    <h4 class="text-white">Date: {{$test_date}} </h4>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- contact area start -->
    <h2 class="text-center text-white bg-dark m-0">{{$test_course}}</h2>
    <div class="row p-5 bg-light" >
        @foreach($results as $result)
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-4  mb20 ">
                <!-- 2 -->
                <div class="themeioan_event shadow">
                    <article>
                        <!-- single event start -->
                        <div class="event-photo">
                            <div class="date">
                                <h4><span>{{$result->rank}}</span> rank</h4>
                            </div>
                            <img src="{{asset('storage/'.$result->image)}}" style="object-fit: cover;height: 300px; width: 100%;" alt="">
                        </div>
                        <div class="event-content">
                            <div class="btn-section">
                                <a href="" class="color-two btn-custom smooth-scroll">{{$result->percentage}}</a>
                            </div>
                            <div class="btn-section">
                                <a href="" class="color-two btn-custom smooth-scroll">{{$result->name}}
                                </a>
                            </div>
                            <div class="btn-section">
                                <a href="" class="color-two btn-custom smooth-scroll">{{$result->state}}</a>
                            </div>
                        </div>
                    </article>
                    <!-- single event end -->
